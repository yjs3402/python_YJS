...
while True:
    ...
    titles_link = driver.find_elements(By.ID, 'video-title-link')
    
    for el in range(len(elem)):
        thumbnail = elem[el].get_attribute('src')
        title = titles_link[el].get_attribute('title')
        links.append(titles_link[el].get_attribute('href'))
