...
for i in range(204,215):
    url = f'https://comic.naver.com/webtoon/detail?titleId=747269&no={i}&week=wed'
    ...
    print(title)
    print(fulltitle)
    page_folder = f"C:/Users/c403/Documents/GitHub/python_YJS/naver_webtoon/{title}"
    if not os.path.exists(page_folder):
        os.mkdir(page_folder)

    page_folder = f"C:/Users/c403/Documents/GitHub/python_YJS/naver_webtoon/{title}/{fulltitle}"
    if not os.path.exists(page_folder):
        os.mkdir(page_folder)
    ...
    for j in range(count):
        ...
        try:
            file_name=  f'./naver_webtoon/{title}/{fulltitle}/{fulltitle}{j+1}{image[-4:]}'
